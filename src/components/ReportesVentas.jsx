...
function BarRow({ label, value, maxValue, color, right }) {
  return (
    <div className="rv-bar-row">
      ...
      <div className="rv-bar-track"><div className="rv-bar-fill" style={{ width: `${pct(value, maxValue)}%`, background: color }} /></div>
        ...
        return (
          <div key={p.nombre + i} className="rv-ranking-row">
            ...
            <div className="rv-ranking-right">
              <div className="rv-bar-track" style={{ width: 50 }}><div className="rv-bar-fill" style={{ width: `${pct(p.totalVendido, max)}%`, background: COLORS[i % COLORS.length] }} /></div>
  ...
  return (
    <>
      ...
      <div className="rv-seccion">
        ...
        <div className="rv-card">
          <div className="rv-tabla-anual-head rv-tabla-6col"><span>Mes</span><span>Ventas</span><span>Recaudado</span><span>Costo</span><span>Ganancia</span><span style={{ textAlign:"right" }}>Margen</span></div>
...
function VistaMensual({ reporte: r }) {
  ...
  const productosConMargenBajo = productos.filter((p)=>{ const m=p.totalRecaudado>0?((p.ganancia||0)/p.totalRecaudado)*100:0; return m<20&&p.totalRecaudado>0; }).length;
      ...
      <div className="rv-dos-col">
        <div className="rv-seccion">
          ...
          <div className="rv-card">
            ...
            {empleados.map((e,i)=>{ const margenEmp=e.totalRecaudado>0?((e.ganancia||0)/e.totalRecaudado)*100:0; return (<div key={e.nombre+i} className="rv-empleado-bloque"><BarRow label={e.nombre} value={e.totalRecaudado||0} maxValue={maxEmp} color={COLORS[i%COLORS.length]} right={`${fmtN(e.totalVentas)} v. · ${fmt(e.ticketPromedio)}`} /><div className="rv-empleado-gan"><GananciaChip ganancia={e.ganancia||0} margen={margenEmp} /></div></div>); })}
          </div>
        </div>
        <div className="rv-seccion">
